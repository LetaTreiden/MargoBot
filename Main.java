...
public class ChatGPTBot extends TelegramLongPollingBot {
        ...
        if (update.hasMessage() && update.getMessage().hasText() || update.hasCallbackQuery()) {
            ...
            switch (userMessage) {
                ...
                case "/fillForm":
                    ...
                    break;
                case "/dayMessage" :
                    getMessageOfTheDay(chatId);
                    break;
                case "/dayCard" :
                    getCardOfTheDay(chatId);
                    break;
    ...
    private void getPrize(String chatId) {
        SendMessage message = new SendMessage();
        message.setChatId(chatId);
        message.setText("Техническое сообщение: пока нет призов, доступных для получения");
    ...
    private void getMessageOfTheDay(String chatId) {
        SendMessage message = new SendMessage();
        message.setChatId(chatId);
        message.setText("Техническое сообщение: сообщений пока нет :(");
    ...
    private void getCardOfTheDay(String chatId) {
        SendMessage message = new SendMessage();
        message.setChatId(chatId);
        message.setText("Техническое сообщение: сообщений пока нет :(");
